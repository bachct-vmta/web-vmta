...
@section('title', __('content::achievement.heading'))

@section('content')
<div class="p-6 max-w-6xl mx-auto" x-data="{ tab: '{{ $activeTab }}' }">
    <h1 class="text-2xl font-semibold mb-4">{{ __('content::achievement.heading') }}</h1>
    ...
    <nav class="flex flex-wrap gap-2 border-b border-slate-200 mb-6" role="tablist">
        @foreach($positions as $p)
            <button type="button" role="tab"
                ...
                {{ __('content::achievement.positions.' . $p->value) }}
            </button>
        @endforeach
        <button type="button" role="tab"
                :aria-selected="tab === 'cases'"
                @click="tab = 'cases'"
                :class="tab === 'cases' ? 'border-blue-600 text-blue-700 font-semibold' : 'border-transparent text-slate-600'"
                class="px-3 py-2 text-sm border-b-2 hover:text-slate-900">
            {{ __('content::achievement.positions.cases') }}
        </button>
    </nav>
    ...
    @foreach($positions as $p)
        ...
        <div x-show="tab === '{{ $p->value }}'" x-cloak>
            @include('content::admin.achievement._fieldset-' . $p->value, ['position' => $p, 'section' => $section])
        </div>
    @endforeach

    <div x-show="tab === 'cases'" x-cloak>
        @include('content::admin.achievement._fieldset-cases', ['cases' => $cases])
    </div>
</div>
@endsection
