<form method="POST" action="{{ route(admin_route_name('about.update'), ['position' => $position->value]) }}"
      class="space-y-4 bg-white border border-slate-200 rounded p-5" data-about-fieldset="how_works">
    @csrf @method('PUT')
    ...
    <div>
        <p class="text-sm font-medium text-slate-700 mb-2">
            {{ __('content::content.about.fields.steps') }} <span class="text-xs text-slate-400">(tối thiểu 1, thêm/xoá tuỳ ý)</span>
        </p>
