<form method="POST" action="{{ route(admin_route_name('about.update'), ['position' => $position->value]) }}"
      class="space-y-4 bg-white border border-slate-200 rounded p-5" data-about-fieldset="core_values">
    @csrf @method('PUT')
    ...
    <div>
        <p class="text-sm font-medium text-slate-700 mb-2">
            {{ __('content::content.about.fields.value_cards') }}
            <span class="text-xs text-slate-400">({{ __('content::content.about.fields.value_cards_hint') }})</span>
        </p>
        @include('content::admin.home._items-repeater', [
            ...
            'variable'    => true,
            'min'         => 0,
            'addLabel'    => __('content::content.about.fields.value_add'),
            'removeLabel' => __('content::content.about.fields.value_remove'),
            'fields'      => [
