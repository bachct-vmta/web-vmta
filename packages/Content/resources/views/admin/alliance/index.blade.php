...
@section('title', __('content::content.alliance.heading'))

@section('content')
<div class="p-6 max-w-6xl mx-auto" x-data="{ tab: '{{ $activeTab }}' }">
    <h1 class="text-2xl font-semibold mb-4">{{ __('content::content.alliance.heading') }}</h1>
    ...
    <nav class="flex flex-wrap gap-2 border-b border-slate-200 mb-6" role="tablist">
        @foreach($positions as $p)
            <button type="button" role="tab"
                ...
                {{ __('content::content.alliance.positions.' . $p->value) }}
            </button>
        @endforeach
    </nav>
    ...
    @foreach($positions as $p)
        ...
        <div x-show="tab === '{{ $p->value }}'" x-cloak>
            @include('content::admin.alliance._fieldset-' . $p->value, ['position' => $p, 'section' => $section])
        </div>
    @endforeach
</div>
@endsection
