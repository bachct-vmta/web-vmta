<form method="POST" action="{{ route(admin_route_name('home.update'), ['position' => $position->value]) }}"
      class="space-y-4 bg-white border border-slate-200 rounded p-5" data-home-fieldset="benefits">
    @csrf @method('PUT')
    ...
    {{-- 2 benefit rows (fixed), each with audience/subtitle/bullets --}}
                ...
                @for($i = 0; $i < 2; $i++)
                    <fieldset class="rounded bg-slate-50 p-3 border border-slate-200">
                        <legend class="text-xs uppercase text-slate-500 px-1">
                            {{ __('content::content.home.fields.item_audience') }} #{{ $i + 1 }}
                        </legend>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1">
                                    {{ __('content::content.home.fields.item_audience') }}
                                </label>
                                <input type="text"
                                       name="translations[{{ $locale }}][items][{{ $i }}][audience]"
                                       value="{{ old("translations.{$locale}.items.{$i}.audience", $existing[$i]['audience'] ?? '') }}"
                                       class="w-full rounded border-slate-300 text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1">
                                    {{ __('content::content.home.fields.item_subtitle') }}
                                </label>
                                <input type="text"
                                       name="translations[{{ $locale }}][items][{{ $i }}][subtitle]"
                                       value="{{ old("translations.{$locale}.items.{$i}.subtitle", $existing[$i]['subtitle'] ?? '') }}"
                                       class="w-full rounded border-slate-300 text-sm">
                            </div>
                        </div>
